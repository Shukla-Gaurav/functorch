...
glu_batch_rule(const Tensor& self, optional<int64_t> self_bdim, int64_t dim) {
  if (!self_bdim) {
    return std::make_tuple(at::glu(self, dim), nullopt);
  }
  ...
  auto dim_ = dim;
  if (dim < 0) { // logic is easier if everything's positive
    dim_ = self.dim() + dim_;
  }
  dim_ = dim_ + 1;
...
std::tuple<Tensor,optional<int64_t>> glu_backward_batch_rule(
  ...
  if (!self_bdim && !grad_output_bdim) {
    return std::make_tuple(at::glu_backward(grad_output, self, dim), nullopt);
  }
